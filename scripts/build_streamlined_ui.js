...
<head>
  ...
  <style>
    /* ==========================================================================
       Streamlined Minimalist Monochrome Theme (Linear / Raycast / Copilot Style)
       ========================================================================== */
    :root {
      --bg-canvas: #fafafa;
      --bg-surface: #ffffff;
      ...
      --shadow-lg: 0 12px 32px 0 rgba(0, 0, 0, 0.09);
    ...
    @media (prefers-color-scheme: dark) {
      :root:not([data-theme="light"]) {
        ...
        --shadow-lg: 0 12px 32px 0 rgba(0, 0, 0, 0.6);
    ...
    * {
    ...
    @media (max-width: 680px) {
      ...
      .composer-card { border-radius: var(--radius-md); }
      .composer-footer { flex-direction: column; align-items: stretch; gap: 0.5rem; }
      .footer-left, .footer-right { justify-content: space-between; width: 100%; }
      .footer-right .btn-primary { flex: 1; justify-content: center; }
      .modal-card { max-width: 95vw !important; margin: 0.5rem; }
      .settings-layout { grid-template-columns: 1fr; }
      .settings-sidebar { flex-direction: row; overflow-x: auto; border-right: none; border-bottom: 1px solid var(--border); }
    }
  </style>
</head>
<body>
  ...
  <header>
    <div class="brand">
      <div class="brand-icon">
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="m12 3-1.9 5.8a2 2 0 0 1-1.3 1.3L3 12l5.8 1.9a2 2 0 0 1 1.3 1.3L12 21l1.9-5.8a2 2 0 0 1 1.3-1.3L21 12l-5.8-1.9a2 2 0 0 1-1.3-1.3Z"/></svg>
      </div>
    ...
    <div class="header-actions">
      ...
      <!-- Consolidated Workspace & Settings Menu -->
      <div class="dropdown">
        <button class="dropdown-btn" onclick="toggleDropdown('workspaceDropdown', event)">
          ...
          <span id="selectedWorkspaceLabel">Settings ▾</span>
        </button>
        <div class="dropdown-menu" id="workspaceDropdown">
          <div class="dropdown-header">Plan Tier</div>
          <button class="dropdown-item" onclick="selectTier('free', 'FREE', event)">Free ($0) — Offline & BYOK</button>
          <button class="dropdown-item active" onclick="selectTier('pro', 'PRO', event)">Pro ($5/mo) — Cloud Models</button>
          <button class="dropdown-item" onclick="selectTier('team', 'TEAM', event)">Team ($15/seat) — Guardrails</button>
          
          <div class="dropdown-divider"></div>
          <div class="dropdown-header">Theme</div>
          <button class="dropdown-item active" onclick="selectTheme('system', event)">System Default</button>
          <button class="dropdown-item" onclick="selectTheme('light', event)">Light</button>
          <button class="dropdown-item" onclick="selectTheme('dark', event)">Dark</button>
          
          <div class="dropdown-divider"></div>
          <div class="dropdown-header">Workspace Tools</div>
          <button class="dropdown-item" onclick="openSettingsTab('modelsTab')">🔑 Models & BYOK Keys...</button>
          <button class="dropdown-item" onclick="openSettingsTab('guardrailsTab')">🏢 Team Guardrails...</button>
          <button class="dropdown-item" onclick="openSettingsTab('analyticsTab')">📊 Token ROI Analytics...</button>
          <button class="dropdown-item" onclick="openSettingsTab('historyTab')">📜 Prompt History & Macros...</button>
          <button class="dropdown-item" onclick="openSettingsTab('shortcutsTab')">⌨️ Keyboard Shortcuts...</button>
        </div>
      </div>
  ...
  <main>
    ...
    <div class="composer-card">
      <div class="composer-header">
        ...
        <div class="composer-stats">
          <span class="stat-pill" id="confidenceBadge">98% Match</span>
          <span class="stat-pill" id="noiseCutLabel">38% Cut</span>
          <span class="stat-pill" id="savingsLabel">$0.00 Saved</span>
        </div>
      </div>

      <!-- Live Voice HUD (visible when recording) -->
      <div class="voice-hud" id="voiceHud">
        <div style="display: flex; align-items: center; gap: 0.5rem;">
          ...
          <span style="font-size: 0.75rem; font-weight: 600; color: #ef4444;" id="voiceStatus">Listening... Speak naturally</span>
        </div>
        <div style="display: flex; align-items: center; gap: 0.5rem;">
          <span style="font-family: var(--font-mono); font-size: 0.72rem; color: var(--text-muted);" id="voiceTimer">00:00</span>
          <button class="btn-icon" onclick="stopRecording()" style="padding: 2px 6px; font-size: 0.7rem;">Done</button>
        </div>
      </div>

      <div class="composer-body">
        <textarea id="rawInput" class="composer-input" placeholder="Click 'Dictate' or type your raw thought stream here... (e.g., 'Um, hey, so I was thinking we should build a script to clean CSV files, actually no, make it a CLI tool in Python.')" style="display: none;"></textarea>
      ...
      <div class="team-rules-inline" id="teamRulesNotice">
        <strong>Team Guardrails Active:</strong> Strict TypeScript (no <code>any</code>), Vitest test suite, Zod validation.
      </div>
      ...
      <div class="composer-footer">
        <div class="footer-left">
          ...
          <div class="dropdown">
            ...
            <div class="dropdown-menu" id="modeDropdown">
              ...
              <button class="dropdown-item active" onclick="selectMode('general', 'Mode: General', event)">General</button>
              ...
              <button class="dropdown-item" onclick="selectMode('sql_data', 'Mode: SQL & Data', event)">SQL & Data</button>
            </div>
          </div>
        ...
        <div class="footer-right">
          <button class="btn-icon" onclick="clearInput()" title="Clear Prompt">Clear</button>
          <button class="btn-icon" id="compileBtn" onclick="triggerCompilation()" title="Re-compile prompt">Re-Compile</button>
    ...
    <div class="dispatch-box" id="sentCard">
      <div class="dispatch-title">
        <span>Dispatched to Target Model</span>
        <span style="font-weight: 400; color: var(--text-faint);" id="sentTimestamp"></span>
      </div>
  ...
  <!-- Toast Element -->
  <div class="toast-box" id="toastBox">Prompt copied to clipboard</div>
  ...
  <div class="modal-overlay" id="commandPaletteModal" onclick="closeOnBackdrop(event, 'commandPaletteModal')">
    <div class="modal-card" style="max-width: 500px;">
      <div class="palette-input-wrap">
        ...
        <input type="text" id="paletteSearch" class="palette-input" placeholder="Type a command or mode... (Esc to close)" oninput="filterPalette()">
      </div>
      <div class="palette-list" id="paletteList">
        <div class="palette-item" onclick="executePalette('mode', 'general')">
          <div class="palette-item-left">General Prompt</div>
          <kbd>1</kbd>
        </div>
        <div class="palette-item" onclick="executePalette('mode', 'code_refactor')">
          <div class="palette-item-left">Code & Refactor</div>
          <kbd>2</kbd>
        </div>
        <div class="palette-item" onclick="executePalette('mode', 'code_review')">
          <div class="palette-item-left">PR / Code Review</div>
          <kbd>3</kbd>
        </div>
        <div class="palette-item" onclick="executePalette('mode', 'architecture_adr')">
          <div class="palette-item-left">Architecture ADR</div>
          <kbd>4</kbd>
        </div>
        <div class="palette-item" onclick="executePalette('mode', 'bug_report')">
          <div class="palette-item-left">Bug Report</div>
          <kbd>5</kbd>
        </div>
        <div class="palette-item" onclick="executePalette('action', 'dictate')">
          <div class="palette-item-left">Start Voice Dictation</div>
          <kbd>Ctrl+Alt+V</kbd>
        </div>
        <div class="palette-item" onclick="executePalette('action', 'settings')">
          <div class="palette-item-left">Open Settings & BYOK Keys</div>
        </div>
        <div class="palette-item" onclick="executePalette('action', 'guardrails')">
          <div class="palette-item-left">Manage Team Guardrails</div>
        </div>
        <div class="palette-item" onclick="executePalette('action', 'analytics')">
          <div class="palette-item-left">View Token ROI Analytics</div>
        </div>
        <div class="palette-item" onclick="executePalette('action', 'history')">
          <div class="palette-item-left">View Prompt History</div>
        </div>
      </div>
    </div>
  </div>

  <!-- Consolidated Settings & Workspace Hub Modal -->
    ...
    <div class="modal-card" style="max-width: 680px;">
      ...
      <div class="settings-layout">
        <div class="settings-sidebar">
          <button class="settings-tab-btn active" id="tabBtnModels" onclick="switchSettingsTab('modelsTab', this)">Models & BYOK</button>
          <button class="settings-tab-btn" id="tabBtnGuardrails" onclick="switchSettingsTab('guardrailsTab', this)">Team Rules</button>
          <button class="settings-tab-btn" id="tabBtnAnalytics" onclick="switchSettingsTab('analyticsTab', this)">ROI Analytics</button>
          <button class="settings-tab-btn" id="tabBtnHistory" onclick="switchSettingsTab('historyTab', this)">Prompt History</button>
          <button class="settings-tab-btn" id="tabBtnShortcuts" onclick="switchSettingsTab('shortcutsTab', this)">Shortcuts</button>
        </div>
        <div class="settings-content">
          ...
          <div id="modelsTab" class="settings-pane active">
            ...
            <div class="form-group">
              <label class="form-label">GCP Vertex Project ID</label>
            ...
            <div style="margin-top: 10px;">
              <button class="btn-icon" onclick="testCloudConnection()">
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
                <span>Test Connection</span>
              </button>
              <span id="connTestStatus" style="font-size: 0.72rem; margin-left: 8px; color: var(--text-muted);"></span>
            </div>
          </div>

          <!-- Team Guardrails Tab -->
          <div id="guardrailsTab" class="settings-pane" style="display: none;">
            <div style="font-size: 0.75rem; font-weight: 600; color: var(--text-main); margin-bottom: 6px;">Active Team Rules (Team Tier: $15/seat)</div>
            <div id="teamRulesConfigList" style="display: flex; flex-direction: column; gap: 0.35rem; margin-bottom: 1rem;">
              <div style="display: flex; align-items: center; justify-content: space-between; background: var(--bg-subtle); padding: 5px 8px; border-radius: 4px; font-size: 0.75rem;">
          ...
          <!-- Analytics Tab -->
          <div id="analyticsTab" class="settings-pane" style="display: none;">
            <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 0.5rem; margin-bottom: 1rem;">
              <div style="background: var(--bg-subtle); padding: 0.65rem; border-radius: var(--radius-sm);">
                <div style="font-size: 0.65rem; color: var(--text-muted); text-transform: uppercase;">Tokens Saved</div>
                <div style="font-size: 1rem; font-weight: 600;" id="analyticsTokensCut">14,820</div>
              </div>
              <div style="background: var(--bg-subtle); padding: 0.65rem; border-radius: var(--radius-sm);">
                <div style="font-size: 0.65rem; color: var(--text-muted); text-transform: uppercase;">Money Saved</div>
                <div style="font-size: 1rem; font-weight: 600;" id="analyticsDollarsSaved">$4.44</div>
              </div>
              <div style="background: var(--bg-subtle); padding: 0.65rem; border-radius: var(--radius-sm);">
                <div style="font-size: 0.65rem; color: var(--text-muted); text-transform: uppercase;">Noise Cut</div>
                <div style="font-size: 1rem; font-weight: 600;">39.2%</div>
              </div>
            </div>
            <div style="font-size: 0.72rem; font-weight: 600; color: var(--text-main); margin-bottom: 0.35rem;">Usage by Mode</div>
            <div style="display: flex; flex-direction: column; gap: 0.35rem; font-size: 0.72rem; color: var(--text-muted);">
              <div>Code & Refactor: 54%</div>
              <div>Bug Report: 26%</div>
              <div>PR Review: 20%</div>
            </div>
          </div>

          <!-- History Tab -->
          <div id="historyTab" class="settings-pane" style="display: none;">
            <div id="settingsHistoryList" style="display: flex; flex-direction: column; gap: 0.4rem;">
  ...
  <footer>
    Prompt Compiler &bull; Speech-to-Prompt Pre-flight Layer
  </footer>
  ...
  <script>
    ...
    function openSettingsTab(tabId) {
      ...
      const tabBtnMap = {
        modelsTab: 'tabBtnModels',
        guardrailsTab: 'tabBtnGuardrails',
        analyticsTab: 'tabBtnAnalytics',
        historyTab: 'tabBtnHistory',
        shortcutsTab: 'tabBtnShortcuts'
      };
      const btnId = tabBtnMap[tabId] || 'tabBtnModels';
      const btn = document.getElementById(btnId);
      if (btn) switchSettingsTab(tabId, btn);
    }
    ...
    function selectTier(tier, label, e) {
      ...
      document.querySelectorAll('#workspaceDropdown .dropdown-item').forEach(btn => {
        if (btn.innerText.includes('Free') || btn.innerText.includes('Pro') || btn.innerText.includes('Team')) {
      ...
      triggerCompilation();
      showToast('Switched to ' + label + ' plan');
    }
    ...
    function switchSettingsTab(tabId, btn) {
      ...
      if (tabId === 'historyTab') renderHistoryInSettings();
    }
    ...
    function executePalette(type, value) {
      ...
      if (type === 'mode') {
        ...
        else if (value === 'settings') openSettingsTab('modelsTab');
        else if (value === 'guardrails') openSettingsTab('guardrailsTab');
        else if (value === 'analytics') openSettingsTab('analyticsTab');
        else if (value === 'history') openSettingsTab('historyTab');
      }
    }
    ...
    function saveToHistory(promptText, mode) {
      let history = [];
      try {
        history = JSON.parse(localStorage.getItem('prompt_compiler_history') || '[]');
      } catch (e) {}
      history.unshift({
        id: Date.now(),
        text: promptText,
      ...
      localStorage.setItem('prompt_compiler_history', JSON.stringify(history));
    }

    function renderHistoryInSettings() {
      let history = [];
      try {
        history = JSON.parse(localStorage.getItem('prompt_compiler_history') || '[]');
      } catch (e) {}
    ...
    function selectTheme(theme, e) {
      ...
      document.querySelectorAll('#workspaceDropdown .dropdown-item').forEach(btn => {
        if (btn.innerText.includes('Default') || btn.innerText === 'Light' || btn.innerText === 'Dark') {
          btn.classList.remove('active');
        }
      });
      if (e && e.currentTarget) e.currentTarget.classList.add('active');
    ...
    function testCloudConnection() {
      ...
      setTimeout(() => {
        statusEl.innerText = '✅ Connection verified & ready';
    ...
    function toggleSpeechRecognition() {
      ...
      if (!recognition) {
        showToast('Voice dictation not supported in browser');
        return;
      }
      if (isRecording) {
        recognition.stop();
        ...
        isRecording = true;
        secondsElapsed = 0;
        document.getElementById('voiceHud').classList.add('active');
    ...
    function stopRecording() {
      ...
      document.getElementById('voiceHud').classList.remove('active');
    ...
    async function triggerCompilation() {
      ...
      try {
        ...
        saveToHistory(data.compiled_prompt, currentMode);
    ...
    function sendMessage() {
      ...
      document.getElementById('sentTimestamp').innerText = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
      card.classList.add('active');
      card.scrollIntoView({ behavior: 'smooth' });
      showToast('Dispatched to Target LLM');
    }
...
console.log('✅ Generated streamlined user-flow public/index.html');
